...
import { setMorphTarget } from '@/features/character-shape/model/use-shape-key-controller';
...
const BLINK_DELAY_BASE_MS = 3000;
...
export const useBlinkAnimation = ({
  ...
  useEffect(() => {
    ...
    const startBlink = () => {
      const delay = BLINK_DELAY_BASE_MS * (1 + Math.random());
