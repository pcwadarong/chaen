...
import { getProjectStaticParams } from './get-project-static-params';
...
describe('getProjectStaticParams', () => {
  ...
  it('공개된 프로젝트 slug를 상세 route params 형식으로 반환한다', async () => {
    const order = vi
      .fn()
      .mockImplementationOnce(() => ({ order }))
      .mockResolvedValueOnce({
        data: [{ slug: 'project-2' }, { slug: 'project-1' }],
        error: null,
      });
  ...
  it('빈 slug는 제거한다', async () => {
    const order = vi
      ...
      .mockImplementationOnce(() => ({ order }))
      .mockResolvedValueOnce({
        data: [{ slug: '  project-1  ' }, { slug: '   ' }, { slug: null }],
        error: null,
      });
    const notSlug = vi.fn(() => ({ order }));
    const notPublishAt = vi.fn(() => ({ not: notSlug }));
    ...
    await expect(getProjectStaticParams()).resolves.toEqual([{ id: 'project-1' }]);
  });
  ...
  it('slug 조회 실패를 에러로 올린다', async () => {
    ...
    await expect(getProjectStaticParams()).rejects.toThrow(
      '[projects] 정적 params slug 조회 실패: boom',
    );
  });
});
