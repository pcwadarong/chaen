...
describe('EditorCore', () => {
  ...
  it(
    'locale 탭 전환 시 제목과 본문 상태가 서로 섞이지 않는다',
    async () => {
      renderEditorCore();
  ...
  it(
    'locale별 textarea scroll 위치를 따로 보존한다',
    async () => {
      renderEditorCore();
  ...
  it(
    '내용이 있는데 제목이 비어 있으면 인라인 에러를 노출한다',
    async () => {
      renderEditorCore();
  ...
  it(
    '데스크톱에서는 편집 pane과 미리보기 pane을 동시에 렌더링한다',
    () => {
      renderEditorCore();
  ...
  it(
    '숨겨진 locale panel의 제목과 설명도 탭 전환 시 높이를 다시 계산한다',
    async () => {
      renderEditorCore({
        initialTranslations: {
          en: {
  ...
  it(
    'toolbar는 active locale textarea에만 값을 적용한다',
    async () => {
      renderEditorCore();
  ...
  it(
    '수동 저장 시 현재 EditorState snapshot을 전달한다',
    async () => {
  ...
  it(
    '저장 실패 시 토스트를 띄우고 dirty 상태를 유지한다',
    async () => {
