...
describe('PublishSlugInput', () => {
  ...
  it('기본 상태에서는 빈 값이어도 인라인 에러를 바로 노출하지 않는다', () => {
    render(<PublishSlugInput onChange={vi.fn()} value="" />);

    expect(screen.queryByText('슬러그를 입력해주세요.')).toBeNull();
  });

  it('showEmptyError가 켜져 있으면 빈 값 에러를 노출한다', () => {
    render(<PublishSlugInput onChange={vi.fn()} showEmptyError value="" />);
  ...
  it('여러 인스턴스를 렌더링해도 각 입력 id가 충돌하지 않는다', () => {
    render(
      <>
        <PublishSlugInput onChange={vi.fn()} value="first-slug" />
        <PublishSlugInput onChange={vi.fn()} value="second-slug" />
      </>,
    );
  ...
  it('사용 가능 확인 시 정규화된 slug로 검사하고 입력값도 정규화한다', async () => {
    ...
    const PublishSlugInputHarness = () => {
      const [value, setValue] = React.useState('');

      return (
        <PublishSlugInput onChange={setValue} onCheckDuplicate={onCheckDuplicate} value={value} />
      );
    };

    render(<PublishSlugInputHarness />);
  ...
  it('중복 확인 전에는 빈 값 경고를 바로 띄우지 않고, 확인 버튼을 누르면 빈 값 에러를 노출한다', async () => {
    ...
    render(<PublishSlugInput onChange={vi.fn()} onCheckDuplicate={onCheckDuplicate} value="" />);
  ...
  it('중복 확인 결과가 사용 가능하면 성공 메시지를 노출한다', async () => {
    ...
    render(
      <PublishSlugInput onChange={vi.fn()} onCheckDuplicate={onCheckDuplicate} value="new-slug" />,
    );
  ...
  it('발행 후 잠금 상태면 읽기 전용 입력으로 렌더링한다', () => {
    render(<PublishSlugInput isPublished onChange={vi.fn()} value="fixed-slug" />);
