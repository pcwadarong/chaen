...
describe('ArticleDetailRoute', () => {
  it('아티클 상세 뷰 엔트리와 데이터를 반환한다', async () => {
    ...
    expect(getArticle).toHaveBeenCalledWith('frontend-performance', 'ko');
  });

  it('데이터가 없으면 notFound를 호출한다', async () => {
    vi.mocked(getArticle).mockResolvedValueOnce(null);
    ...
    expect(getArticle).toHaveBeenCalledWith('missing-article', 'ko');
