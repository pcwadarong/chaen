...
각 브랜치의 작업 지시서는 같은 폴더의 번호별 파일에 있다. 워크트리를 따로 파서 하나씩 진행하고, 아래 순서로 병합하면 충돌이 없다.
...
- 3번이 가장 작고 다른 브랜치가 참조할 경로 상수를 만든다 → 먼저 넣는다.
- 1번은 `public/` 바이너리를 전부 갈아엎으므로 소스 변경이 큰 것들 뒤에 둔다.
- 6번은 1번이 정리한 결과 위에서만 의미가 있다.

**1·2·3번이 서로 독립적이지 않은 유일한 지점**: 셋 다 `src/entities/scene/model/preloadGLB.ts`를 건드릴 수 있다. 3번이 이 파일을 `scene-asset-paths.ts`로 재편하므로, 1·2번은 이 파일의 **경로 정의를 수정하지 말고** 소비만 할 것.
