...
전후를 기록하고 PR에 표로 넣어라. 목표 ≈50MB. 3. `gltf-transform inspect`로 노드 22개 이름·애니메이션 4클립·모프 타깃 보존 확인
(스킬 Step 4). `assets-src/`의 베이스라인과 대조. 4. **화질 비교** — 전후 같은 카메라 위치 스크린샷:
얼굴 클로즈업(피부·눈) / 머리카락 알파 경계 / 옷 / 헤드폰·노트북 금속면 / 소품 나뭇결 5. **Safari(iOS 포함) 필수.** 지원 포맷이 달라 Safari에서만 깨지는 사례가 흔하다. 6. `pnpm run test:browser`
