...
즉 지금은 “새 캔버스를 앞에 세워두고, 뒤쪽에 예전 캔버스를 보험처럼 남겨둔 상태”입니다.  
이 문서는 그 보험을 언제, 어떤 순서로 걷어낼지 정리합니다.

## 결론

다음 순서가 맞습니다.

1. 운영 환경에서 shadow-first 읽기 경로를 충분히 검증
2. 코드에서 legacy relation fallback 분기 제거
...
- 동시에 장애 대비용으로 기존 `articles`, `projects`, `article_tags`, `project_tags` fallback은 일부 남아 있습니다.
- 이 상태에서 canonical rename을 먼저 하면, fallback 경로가 섞여 예기치 않은 런타임 오류가 날 수 있습니다.

## 현재 남아 있는 legacy 의존

### 1. legacy relation table fallback
...
- `article_tags_v2`, `project_tags_v2`가 없을 때만 기존 `article_tags`, `project_tags`를 읽는 안전장치

제거 시점:

- relation table shadow schema와 태그 집계 결과를 운영 환경에서 충분히 검증한 뒤
...
2. legacy fallback 분기 제거
3. `article_tags`, `project_tags` 직접 조회 fallback 제거

### 권장

1. `resolveLocaleAwareData`를 content API에서 더 단순한 fallback 정책으로 줄이기
2. `*_legacy`를 참조하는 테스트 fixture 추가
3. revalidate 태그와 캐시 키가 cutover 뒤에도 변하지 않는지 점검
